...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    if (csrf_verify()) {
        ...
        $file = $stmt->fetch();
        if ($file && file_exists(BASE_PATH . '/' . $file['filepath'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_media'])) {
    ...
    foreach ($_FILES['files']['tmp_name'] as $idx => $tmp) {
        ...
        if ($error !== UPLOAD_ERR_OK) { $errors[] = "$name: feltöltési hiba."; continue; }
        if ($size > $max_size)        { $errors[] = "$name: túl nagy (max 5MB)."; continue; }

        // MIME típus ellenőrzése a fájl tartalmából (nem a böngésző által küldött értékből!)
        ...
        if (move_uploaded_file($tmp, $dest)) {
            $pdo->prepare("INSERT INTO media (filename, filepath, filetype, filesize, alt_text)
                           VALUES (?, ?, ?, ?, ?)")
                ->execute([$newname, $filepath, $real_type, $size, '']);
            $uploaded++;
        } else {
            $errors[] = "$name: nem sikerült a feltöltés. (Ellenőrizd a mappa jogosultságát!)";
        }
    }
}
...
<div class="media-grid" id="mediaGrid">
    ...
    <div class="media-item">
        <div class="media-thumb" onclick="openMediaDetail(<?= htmlspecialchars(json_encode($f), ENT_QUOTES) ?>)">
            <?php if ($is_image): ?>
                <img src="<?= BASE_URL . '/' . e($f['filepath']) ?>" alt="<?= e($f['alt_text']) ?>" loading="lazy">
            <?php else: ?>
        ...
        <div class="media-actions">
            <button class="action-btn view" onclick="copyUrl('<?= BASE_URL . '/' . e($f['filepath']) ?>')" title="URL másolása">
...
function openMediaDetail(f) {
    ...
    document.getElementById('mediaDetailBody').innerHTML = `
        ${isImage ? `<div style="text-align:center;margin-bottom:16px;"><img src="<?= BASE_URL ?>/${f.filepath}" style="max-width:100%;max-height:300px;border-radius:8px;object-fit:contain;" alt=""></div>` : ''}
        ...
        <div class="detail-row">
            ...
            <span class="detail-value" style="word-break:break-all;font-size:12px;">
                <?= BASE_URL ?>/${f.filepath}
                <button onclick="copyUrl('<?= BASE_URL ?>/${f.filepath}')" class="btn btn-secondary" style="padding:3px 8px;font-size:11px;margin-left:6px;">
